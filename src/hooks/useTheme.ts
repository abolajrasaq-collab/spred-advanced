...
export default function () {
  ...
  const isDark = useSelector((state: { theme: ThemeState }) =>
    state.theme.systemMode ? null : state.theme.mode === 'dark',
  );
...
const mergeVariables = (
  ...
  // Ensure themeConfig and darkThemeConfig are objects
  const safeThemeConfig = themeConfig || {};
  const safeDarkThemeConfig = darkThemeConfig || {};

  return Object.entries(DefaultVariables).reduce((acc, [group, vars]) => {
    const theme:
      | Record<keyof typeof DefaultVariables, typeof vars>
      | undefined = (safeThemeConfig as any)[group];
    const darkTheme:
      | Record<keyof typeof DefaultVariables, typeof vars>
      | undefined = (safeDarkThemeConfig as any)[group];
