...
class P2pFileTransferModule(
  ...
  fun connectWithConfig(readableMap: ReadableMap?, callback: Callback) {
    try {
      ...
      if (readableMap == null) {
        callback.invoke(-1, "Connection config cannot be null")
        return
      }

      val bundle = Arguments.toBundle(readableMap)
      if (bundle == null) {
        callback.invoke(-1, "Failed to process connection config")
        return
      }

      val deviceAddress = bundle.getString("deviceAddress")
      if (deviceAddress == null || deviceAddress.isEmpty()) {
        callback.invoke(-1, "Device address is required")
        return
      }
      ...
      if (bundle.containsKey("groupOwnerIntent")) {
        try {
          ...
          Log.w(NAME, "Invalid groupOwnerIntent value, using default", e)
        }
      }
      ...
      manager?.connect(
        ...
        callback.invoke(-1, "WiFi P2P manager not available")
      }
    } catch (e: Exception) {
      Log.e(NAME, "Error in connectWithConfig", e)
      callback.invoke(-1, "Connection failed: ${e.message}")
    }
  }
