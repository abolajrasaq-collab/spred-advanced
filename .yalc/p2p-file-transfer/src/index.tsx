...
export const connectWithConfig = (config: ConnectionArgs): Promise<void> =>
  new Promise((resolve, reject) => {
    // Ensure config is a valid object before passing to native module
    if (!config || typeof config !== 'object') {
      reject(new Error('Invalid connection config: config must be an object'));
      return;
    }

    if (!config.deviceAddress || typeof config.deviceAddress !== 'string') {
      reject(new Error('Invalid connection config: deviceAddress is required and must be a string'));
      return;
    }
